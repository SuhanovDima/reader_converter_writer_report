...
public class StatisticsRepository {
    ...
    private static final String SELECT_SQL_BY_INTERVAL =
            "SELECT MAX(statistics_date) AS statistics_date, SUM(clicks_total) AS clicks_total, SUM(payment_total) AS payment_total " +
                    "FROM statistics WHERE statistics_date >= ? AND statistics_date < ?";
    ...
    public Statistics getStatisticsByInterval(LocalDate from, LocalDate to) {
        List<Statistics> rows = jdbcOperations.query(SELECT_SQL_BY_INTERVAL, new Object[] {from, to}, rowMapper);
        return rows.isEmpty() ? null : rows.get(0);
    }
